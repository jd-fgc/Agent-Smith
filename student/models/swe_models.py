...
class SWEBenchTaskInput(BaseModel):
    ...
    hints_text: str = Field(
        default="", description="Optional hints about the issue \
            (may be empty)")
    repo: str = Field(
        default="", description="Repository name \
            (e.g.,'sympy/sympy')")
...
class StepMetrics(BaseModel):
    ...
    request_time_ms: float = Field(
        ..., description="Wall-clock time for the LLM API \
    ...
    sandbox_input: str = Field(
        default="", description="Python code sent to \
            the sandbox for execution")
    sandbox_output: str = Field(
        default="", description="Sandbox execution result \
            (stdout/stderr/error message)")
    retries: int = Field(
        default=0, description="Number of LLM API retries before getting a \
            successful response (0 = first attempt succeeded)")
...
class SolutionOutput(BaseModel):
    ...
    benchmark: str = Field(..., description="Benchmark type: \
        'mbpp' or 'swebench'")
    success: bool = Field(..., description="Whether the agent believes \
        it solved the task")
    solution: str = Field(..., description="For MBPP: \
        the Python function code. For SWE-bench: the git patch (diff)")
    ...
    total_output_tokens: int = Field(..., description="Sum of output_tokens \
        across all steps")
    total_time_seconds: float = Field(..., description="Wall-clock time \
        from agent start to finish")
    steps: List[StepMetrics] = Field(default_factory=list,
                                     description="Per-step metrics, \
                                         one entry per agent iteration")
    system_prompt: str = Field(default="", description="Full system prompt \
        sent to the LLM (for provenance checking)")
